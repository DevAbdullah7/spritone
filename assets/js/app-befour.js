...
async function handleFiles(files) {
    ...
    // التحديث والتوليد لمرة واحدة بعد إكمال معالجة الدفعة كاملة
    if (addedAny) {
        renderIcons();
        generateSprite();
    }
...
function renderIcons() {
    ...
    if (icons.length > 0) {
        // استخدام DocumentFragment لتقليل عمليات الـ Re-flow في الـ DOM
                ...
                <div class="action-call-buttons">
                    <div class="input" style="display: none;">
                        <div class="inputContainer">
                            <input type="text" name="icon-name" placeholder="Enter Icon Name" oninput="inputHandlling(this), this.value = this.value.replace(/[^a-zA-Z0-9\s-]/g, '')">
                        </div>
                    </div>
...
function editIconName(btn, index) {
    ...
    // إظهار الحقل وتعبئته بالاسم الحالي كقيمة افتراضية
    inputWrapper.style.display = 'block';
    inputElement.value = icons[index].id;
    inputElement.focus();

    // تغيير أيقونة الزر إلى صح
    btn.innerHTML = `
        <svg width="24" height="24">
    ...
    // إسناد أحداث الحفظ مباشرة لمنع تكرار الـ Event Listeners
    btn.onclick = (e) => {
        e.stopPropagation();
...
function updateIconName(index, newName) {
    ...
    // - استبدال أي حرف ليس إنجليزياً أو رقماً أو مسافة بـ لا شيء (إزالة الأحرف العربية والرموز).
    // - استبدال المسافات بالشرطات.
    // - إزالة الشرطات المتكررة.
    const cleanId = newName
        .replace(/\.svg$/i, '')
        .toLowerCase()
        .replace(/[^a-z0-9\s-]/g, '') // إزالة كل ما هو ليس حرف إنجليزي أو رقم أو مسافة أو شرطة
        .trim()
        .replace(/\s+/g, '-')          // تحويل المسافات إلى شرطات
        .replace(/-+/g, '-');          // منع تكرار الشرطات مثل (icon--name)

    // 2. إذا أصبح الاسم فارغاً (مثلاً لو أ دخل المستخدم حروف عربية فقط)
    if (!cleanId) {
        alert('يرجى إدخال اسم باللغة الإنجليزية فقط (أحرف إنجليزية، أرقام، أو شرطات).');
        renderIcons(); // إعادة الرسم لاستعادة الاسم الأصلي
        return;
    }

    // 3. التحقق من تكرار الاسم
    ...
    // 4. تحديث البيانات وإعادة التوليد
...
// document.querySelector("#iconsList").childNodes[4]
